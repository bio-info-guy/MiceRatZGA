...
prepareCount <- function(cds, dirc)
{
  ...
  if('tpm' %in% names(cds) & !('ct' %in% names(cds))){
    ct <- cds[['salmon']]$gene$counts[,cells]
    cds[['ct']] <- list()
    cds[['ct']][['bio']] <- ct[which(!grepl("ERCC", row.names(ct))),]

    cds[['ct']][['spike']] <- ct[which(grepl("ERCC", row.names(ct))),]
...
import_wig <- function (file, file2, format = c("BED", "bedGraph", "WIG", 
                                  "BigWig"), ranges = GRanges(), ignore.strand = TRUE) 
{
  if (missing(file)) 
  ...
  getWigInfo <- function(firstline) {
    ...
    firstline <- do.call(rbind, strsplit(firstline, "=", 
                                         fixed = TRUE))
    firstline <- firstline[match(c("chrom", "span", "start", 
                                   "step"), firstline[, 1]), ]
    info <- c(structure, firstline[, 2])
    names(info) <- c("structure", "chrom", "span", "start", 
                     "step")
    return(info)
  }
  readWIG <- function(buf, lastWigInfo = NULL) {
    buf <- gsub("^\\s+", "", buf)
    buf <- gsub("\\s+$", "", buf)
    buf <- buf[grepl("^(variableStep|fixedStep|([0-9]+))", 
                     buf)]
    ...
    if (wiginfo["structure"] == "fixedStep") {
      lastWigInfo <- gsub("start=\\d+(\\s)", paste("start=", 
                                                   as.numeric(r[nrow(r), 3]) + 1, "\\1", sep = ""), 
                          lastWigInfo)
    }
    buf <- lapply(buf, "[", -1)
    ...
    list(gr = GRanges(seqnames = r[, 1], ranges = IRanges(start = as.numeric(r[, 
                                                                               2]), end = as.numeric(r[, 3])), score = buf, span = as.numeric(r[, 
                                                                                                                                                4]), step = as.numeric(r[, 5]), structure = r[, 
                                                                                                                                                                                              6]), lastWigInfo = lastWigInfo)
  }
  readBED <- function(buf) {
    ...
    if (all(len == maxLen)) {
      ...
      NAs <- rep("", maxLen)
      buf <- do.call(rbind, lapply(buf, function(.ele) c(.ele, 
                                                         NAs)[1:maxLen]))
    }
    if (ncol(buf) == 3) 
      buf <- cbind(buf, ".")
    ...
    if (nrow(buf) > 0) {
      GRanges(seqnames = buf[, 1], ranges = IRanges(start = as.numeric(buf[, 
                                                                           2]) + 1, end = as.numeric(buf[, 3])), strand = buf[, 
                                                                                                                              6], score = as.numeric(buf[, 5]))
    }
    else {
      GRanges(seqnames = buf[, 1], ranges = IRanges(start = as.numeric(buf[, 
                                                                           2]), end = as.numeric(buf[, 3])), strand = buf[, 
                                                                                                                          6], score = as.numeric(buf[, 5]))
    }
  }
  readFourCols <- function(buf) {
    ...
    if (nrow(buf) > 0) {
      GRanges(seqnames = buf[, 1], ranges = IRanges(start = as.numeric(buf[, 
                                                                           2]) + 1, end = as.numeric(buf[, 3])), score = as.numeric(buf[, 
                                                                                                                                        4]))
    }
    else {
      GRanges(seqnames = buf[, 1], ranges = IRanges(start = as.numeric(buf[, 
                                                                           2]), end = as.numeric(buf[, 3])), score = as.numeric(buf[, 
                                                                                                                                    4]))
    }
  }
  ...
  readFile <- function(file, format, FUN) {
    if (format == "WIG") {
      ...
      if (s < 1e+08) {
        ...
        con <- file(file, open = "r")
        on.exit(close(con))
        while (length(buf <- readLines(con, n = 1e+06, 
                                       warn = FALSE)) > 0) {
          buf <- FUN(buf)
          if (length(res) < 1) {
            res <- buf
  ...
  if (!missing(file2)) {
    ...
    return(new("track", dat = trackViewer:::orderedGR(res), dat2 = trackViewer:::orderedGR(res2), 
               type = "data", format = format))
  }
  else {
    return(new("track", dat = trackViewer:::orderedGR(res), type = "data", 
               format = format))
  }
}
...
enrich_CP <- function(ora_genes, organisms, universe = NULL, logFC=NULL, GSE = F, GO_BP_only = F, enrich_all = T){
  ...
  if(GO_BP_only | enrich_all){
    GSE_results[['GO_BP_ora']] <- tryCatch({setReadable(enrichGO(gene = oraL,
                                                                 ...
                                                                 qvalueCutoff = 0.05), OrgDb = orgdb)},error=function(cond){return(NULL)})
  }
  if(!GO_BP_only){
    GSE_results[["WKP_ora"]] <- tryCatch({setReadable(enrichWP(oraL, organism = orgname, maxGSSize = 500, minGSSize = 10, universe = universe, pvalueCutoff  = 0.1, qvalueCutoff = 0.05 ), OrgDb = orgdb)},error=function(cond){return(NULL)})
    ...
    GSE_results[['KEGG_ora']] <- tryCatch({setReadable(enrichKEGG(gene = oraL, universe = universe,
                                                                  ...
                                                                  pvalueCutoff  = 0.1,qvalueCutoff = 0.05), OrgDb = orgdb, keyType = 'ENTREZID')},error=function(cond){return(NULL)})
    
    
    GSE_results[['MKEGG_ora']] <- tryCatch({setReadable(enrichMKEGG(gene = oraL, universe = universe, maxGSSize = 500,minGSSize = 10, 
                                                                    organism = orgabv, pvalueCutoff  = 0.1,qvalueCutoff = 0.05), OrgDb = orgdb, keyType = 'ENTREZID')},error=function(cond){return(NULL)})
    
    # Reactome Enrichment
    GSE_results[['REACT_ora']] <- tryCatch({setReadable(enrichPathway(gene=oraL,organism = organisms, maxGSSize = 500,minGSSize = 10,  universe = universe, pvalueCutoff  = 0.1,qvalueCutoff = 0.05), OrgDb = orgdb, keyType = 'ENTREZID')},error=function(cond){return(NULL)})
  }
  ...
  if(!is.null(logFC)){ 
    ...
    egdf <- bitr(names(gse_list), 'ALIAS', 'ENTREZID', orgdb) %>% distinct(eval(as.name('ALIAS')), .keep_all = T) %>% data.frame(row.names = 1)
...
gse_CP <- function(ora_genes, organisms, logFC=NULL, GSE = F, simplify_go = T, combine = T, simple_combine = T){
  ...
  if(!is.null(logFC)){ 
    ...
    if(GSE){
      ...
      GSE_results[['REACT_gse']] <- setReadable(gsePathway(geneList =  gse_list, organism = organisms, nPermSimple = 100000, minGSSize = 10, maxGSSize = 250, 
...
deg_utr2 <- function(file, ct, compare, meta, impute = F, method = 'fisher.test', alpha = 0.05, combine_p = NULL, diff_thresh = 0.05){
  require(betareg)
  require(tidyr)
  require(qvalue)
  require(pbapply)
...
plotDEXSeqDTU <- function(expData = NULL, geneID = NULL, samps = NULL, isProportion = FALSE) {
  ...
  p = ggplot(sub, aes(tid, value, color = group, fill = group)) +
    ...
    geom_quasirandom( size = 1, dodge.width = 0.8, alpha = 0.8) + theme_bw() +
    ggtitle(geneID) + xlab("Transcripts")+theme(axis.text.x = element_text(angle = 15, vjust = 0.5, hjust=0.5, size = 8), axis.title = element_text(size = 12), title = element_text(size = 12))
...
cp_tree_ridge_plot <- function(res, n_cat = 50, nclust = 8, alpha = 0.05, geneSet = NULL){
  ...
  res_tree <- addSmallLegend(enrichplot::treeplot(res, showCategory = min(n_cat, nrow(res@result)), 
                               ...
                               geom_tiplab(offset = 0.8, hjust = 0, 
                                           show.legend = FALSE, 
                                           align=TRUE, size = 3.5, lineheight = 0.75)+xlim(c(0,20))+scale_size(name = "number of genes",
                                                                                            range = c(1, 4)))
  tree <- res_tree
