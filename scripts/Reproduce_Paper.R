...
ggsave('./mouse_volcano_mast.png', width = 4, height = 4)
volcano_plot(mast_rat$ratEgg_v_ratZygote$DESig, pval_col = 'fdr',fc = log2(2), top_genes = c('Rps20', 'Rps2', 'Rps15', 'Rplp1','Hist2h3c2','H2ac1', 'H2ax', 'Gata3',
                                                                                                   'Sdhc','Aco2','Idh1','Sdhd','Pdha1','Dlst','Ndufab1','Ndufc1'))+theme(axis.text.y = element_text(size = 15), axis.text.x = element_text(size = 15))
ggsave('./rat_volcano_mast.png', width = 4, height = 4)
...
percentage_summary <- data.frame(percentages = c(perc_mouse_genes_unspliced, perc_rat_genes_unspliced),
                                 Species = c(rep('Mouse', length(perc_mouse_genes_unspliced)), rep('Rat', length(perc_rat_genes_unspliced))),
                                 cellType = c(mouse$meta[names(perc_mouse_genes_unspliced),'cellType'], rat$meta[names(perc_rat_genes_unspliced),'cellType'])) %>% group_by(cellType) %>% summarise(Percentage = mean(percentages*100), sd_Perc = sd(percentages*100), Species = unique(Species))
...
test_vp <- plot_range_coverage(range = 'NC_000070.7:143,920,015-143,923,663',txdb=mouse_gtf, c('./dataset/mouse_apa_res/MF.sf.bw', './dataset/mouse_apa_res/MU.sf.bw'), log = T, cols = DOT_COLOR[c('mouseZygote', 'mouseEgg')], cell_names = c('Mouse zygote', 'Mouse oocyte'), file_name_suffix = 'mouse', y_lim = c(0.08, 0.73))
test_vp <- plot_range_coverage(range = 'NC_000073.7:15,130,780-15,132,520', txdb=mouse_gtf, bw_file_list= c('./dataset/mouse_apa_res/MF.sf.bw', './dataset/mouse_apa_res/MU.sf.bw'), log = T, cols = DOT_COLOR[c('mouseZygote', 'mouseEgg')], cell_names = c('Mouse zygote', 'Mouse oocyte'), file_name_suffix = 'mouse', y_lim = c(0.08, 0.73))
test_vp <- plot_range_coverage(range = 'NC_051349.1:35,194,801-35,196,994', txdb=rat_gtf, bw_file_list= c('./dataset/rat_apa_res/RF.sf.bw', './dataset/rat_apa_res/RU.sf.bw'), log = T, cols = DOT_COLOR[c('ratZygote', 'ratEgg')], cell_names = c('Rat zygote', 'Rat oocyte'), file_name_suffix = 'rat', y_lim = c(0.08, 0.73))
...
mouse_dapars_sf_st25 <- deg_utr('./dataset/mouse_apa_res/combined.txt', mouse$ct$bio, c('mouseEgg', 'mouseZygote'), mouse$meta)
