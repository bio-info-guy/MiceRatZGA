...
mouse <- prepareCount(mouse, dirc = './mouse_data/')
mouse <- prepareGeneFeatures(mouse, gtf_file = './dataset/mouse_data/mouse_spike.gtf')
...
mouse$meta$rRNA_rate <- colSums(mouse$tpm$bio[row.names(subset(mouse$features_tpm, gene_biotype == 'rRNA')),row.names(mouse$meta)])/colSums(mouse$tpm$bio[,row.names(mouse$meta)])
...
rat <- prepareMeta(list(), './rat_data/', QC = F,filter_samples = c('RU14','RF11','RF13', 'RF7', 'RF16', 'RF8'))
...
# plot UMAPs 
umap_config= list(n_neighbors = 10, min_dist = 0.5, metric='euclidean', seed = 123456)
...
ggsave('./mouse_umap.png', height = 2, width = 2)
...
ggsave('./rat_umap.png', height = 2, width = 2)
...
ggplot(deg_summary, aes(fill=deg, y=value, x=Species, label = value)) + 
  ...
  scale_y_continuous(name="Number of DEGs", labels = label_number(scale_cut = cut_short_scale()))+scale_fill_manual(values = c('#6EE2FF', '#FF410D'))
...
test_vp <- plot_utr_coverage('Eif4e', utr_res = mouse_dapars_sf_st25$gene_res, mouse_gtf, c('./dataset/igv/MF.sf.average.sorted.bw', './dataset/igv/MU.sf.average.sorted.bw'), cols= c(DOT_COLOR['mouseZygote'], DOT_COLOR['mouseEgg']),cell_names = c('Mouse zygote', 'Mouse oocyte'), file_name_suffix = 'mouse')
